...
const char *JS_SHOULD_DO_LOOP =
    "function shouldDoLoop(i, iter, Tr, Ti, limit) {\n"
    "    if (i >= iter) { return false; }\n"
    "    if ((Tr + Ti) > (limit * limit)) { return false; }\n"
    "    return true;\n"
    "}\n"
    "(shouldDoLoop)\n";
...
int (*shouldDoLoop)(int, int, double, double, double);
...
void setup(void *arg)
{
  doLoop = polyglot_eval("python", PY_DO_LOOP);
  shouldDoLoop = polyglot_eval("js", JS_SHOULD_DO_LOOP);
}
