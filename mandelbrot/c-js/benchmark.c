...
const char *JS_DO_LOOP =
    "("
    "function doLoop(getValue, setValue, ZrPtr, ZiPtr, CrPtr, CiPtr, TrPtr, "
    "TiPtr) {\n"
    "\n"
    "    var Zi = 2.0 * getValue(ZrPtr) * getValue(ZiPtr) + getValue(CiPtr);\n"
    "    setValue(ZiPtr, Zi);\n"
    "\n"
    "    var Zr = getValue(TrPtr) - getValue(TiPtr) + getValue(CrPtr);"
    "    setValue(ZrPtr, Zr);\n"
    "\n"
    "    var Tr = Zr * Zr;\n"
    "    setValue(TrPtr, Tr);\n"
