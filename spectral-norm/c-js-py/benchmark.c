...
{
  int i, j;
  if (polyglot_is_value(u))
  {
    u = polyglot_as_double_array(u);
  }
  if (polyglot_is_value(Au))
  {
    Au = polyglot_as_double_array(Au);
  }
...
void setup(void *arg)
{
  eval_A = polyglot_eval("python",
                         "def eval_A(i, j):\n"
                         "    return 1.0 / ((i + j) * (i + j + 1.0) / 2 + i + 1.0)\n"
                         "\n"
                         "eval_A\n");
  eval_AtA_times_u = polyglot_eval("js",
                                   "function eval_AtA_times_u(N, u, AtAu, eval_A_times_u, eval_At_times_u) {\n"
                                   "    var v = [];\n"
...
double spectralNorm(int N)
{
  ...
  for (i = 0; i < 10; i++)
  {
    eval_AtA_times_u(N, u, v, &eval_A_times_u, &eval_At_times_u);
    eval_AtA_times_u(N, v, u, &eval_A_times_u, &eval_At_times_u);
  }
  vBv = vv = 0;
  for (i = 0; i < N; i++)
  {
