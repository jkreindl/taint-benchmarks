...
const char *PY_CREATE_ARRAY = "def createPyArray():\n"
                              "    arr = []\n"
                              "    for i in range(5):\n"
                              "        arr.append(None)\n"
                              "    return arr\n\n"
...
{
  void *pyArr = createPyArray();
  double *pyArrAsDoubleArr = polyglot_as_double_array(pyArr);
  return pyArrAsDoubleArr;
}
double *x = ((void *)0);
double *y = ((void *)0);
double *z = ((void *)0);
...
void setup(void *arg)
{
  createPyArray = polyglot_eval("python", PY_CREATE_ARRAY);
}
