...
const char *JS_CREATE_BODY = "(function() { return { x: 0.0, y: 0.0, z: 0.0, "
                             "vx: 0.0, vy: 0.0, vz: 0.0, mass: 0.0 }; })\n";

struct Body *(*createBody)(void);

void setup(void *arg) { createBody = polyglot_eval("js", JS_CREATE_BODY); }
...
void init() {
  ...
  for (int i = 0; i < NBODIES; i++)
    bodies[i] = polyglot_as_Body(createBody());
